...
from ui.widgets import StatusBarProgress, OutlineWidget, ProblemsDelegate, Minimap
from ui.dialogs import (
    NewFileDialog, PackageDialog, DeployDialog, ProgressDialog,
    PythonVersionDialog, AdvancedFindSimilarDialog, FindSimilarDialog
)
...
class IDE(QMainWindow):
    ...
    def _create_fallback_settings_manager(self):
        """Cria um SettingsManager básico como fallback"""
        ...
        return FallbackSettingsManager()
    ...
    def setup_project_menu(self, menu):
        ...
        actions = [
            ("📦 Novo Projeto (Básico)", None, self.create_project),
            ("🎯 Novo Projeto com Seleção de Python", None, self.create_project_with_python_selection),  # NOVA OPÇÃO
            ("📂 Abrir Projeto", None, self.set_project),
            ("🔧 Configurar Projeto", None, self.configure_project),
            ("---", None, None),
            ("🐍 Criar Virtualenv (Seletor)", None, self.create_venv_with_python_selector),
            ("🐍 Criar Virtualenv (Automático)", None, self.create_venv_dynamic),
    ...
    def setup_tools_menu(self, menu):
        ...
        select_python_venv_action = QAction("🎯 Criar Venv (Selecionar Python)", self)
        select_python_venv_action.triggered.connect(self.create_venv_with_python_selector)
        python_menu.addAction(select_python_venv_action)
        
        select_python_action = QAction("🎯 Selecionar Python para VMs", self)
    ...
    def run_linter(self):
        ...
        try:
            current_widget = self.tab_widget.currentWidget()
            if not current_widget:
                QMessageBox.information(self, "Informação", "Nenhum arquivo aberto.")
                return
            ...
            # Salvar arquivo antes de analisar
            self.save_file()
    
            self.output_tabs.setCurrentWidget(self.lint_text)
            self.lint_text.clear()
            self.lint_text.appendPlainText("🔍 Analisando código...\n")
            self.lint_text.appendPlainText("=" * 50 + "\n")
    
            python_exec = self.get_python_executable()
    
            # Tentar diferentes linters
            linter_commands = [
                [python_exec, "-m", "pylint", file_path],
                [python_exec, "-m", "pyflakes", file_path],
                [python_exec, "-m", "flake8", file_path]
            ]
            ...
            for cmd in linter_commands:
                try:
                    ...
                    self.lint_text.appendPlainText(f"Comando: {' '.join(cmd)}\n")
    
                    if result.stdout:
                        self.lint_text.appendPlainText("Saída:\n")
                        self.lint_text.appendPlainText(result.stdout)
                    
                    if result.stderr:
                        self.lint_text.appendPlainText("Erros:\n")
                        self.lint_text.appendPlainText(result.stderr)
    
                    if result.returncode == 0:
                        self.lint_text.appendPlainText(f"\n✅ Análise concluída com {cmd[2]}!")
                        break
                    else:
                        self.lint_text.appendPlainText(f"\n⚠️ {cmd[2]} encontrou problemas\n")
                        self.lint_text.appendPlainText("-" * 30 + "\n")
    
                except subprocess.TimeoutExpired:
                    self.lint_text.appendPlainText(f"⏰ Timeout no {cmd[2]}\n")
                except Exception as e:
                    self.lint_text.appendPlainText(f"❌ Erro com {cmd[2]}: {str(e)}\n")
    
        except Exception as e:
            self.debug_log(f"❌ Erro ao executar linter: {e}", "ERROR")
            self.lint_text.appendPlainText(f"💥 Erro inesperado: {str(e)}")
    def create_project(self):
        """Cria um novo projeto com estrutura completa E seleção de Python para virtualenv"""
        try:
            ...
            if os.path.exists(project_full_path):
                reply = QMessageBox.question(
                    self,
                    "Diretório já existe",
                    f"O diretório '{project_name}' já existe.\n\nDeseja usar este diretório?",
        ...
        *.swo
        ...
        """)
                ...
                QMessageBox.warning(
                    self, "Erro", f"Não foi possível criar a estrutura do projeto:\n{str(e)}"
                )
                return
            ...
            # ✅ PERGUNTAR SE QUER CRIAR VIRTUALENV COM SELEÇÃO DE PYTHON
            reply = QMessageBox.question(
                self,
                "Virtualenv",
            ...
            if reply == QMessageBox.Yes:
                # 🎯 OFERECER SELEÇÃO DE PYTHON
                python_choice = self._offer_python_selection_for_project(project_name)
                
                if python_choice:
                    python_exec, python_info = python_choice
                    # Criar venv com Python selecionado
                    QTimer.singleShot(1000, lambda: self._create_venv_for_new_project(project_full_path, python_exec, python_info))
                else:
                    # Usar método automático se o usuário cancelou a seleção
                    QTimer.singleShot(1000, lambda: self.create_venv_dynamic())
    ...
    def create_project_with_python_selection(self):
        """Cria um novo projeto com seleção da versão do Python"""
        try:
            # Obter nome do projeto
            project_name, ok = QInputDialog.getText(
                self,
                "Novo Projeto com Seleção de Python", 
                "Nome do projeto:",
                text="meu_projeto"
            )
            ...
            # 🐍 DETECTAR VERSÕES DO PYTHON INSTALADAS
            python_versions = self._detect_all_python_versions()
            
            if not python_versions:
                QMessageBox.warning(
                    self,
                    "Python não encontrado",
                    "Nenhuma versão do Python foi detectada no sistema.\n\n"
                    "O projeto será criado sem configuração específica de Python."
                )
                # Criar projeto básico
                self._create_basic_project_structure(project_full_path, project_name)
                return
    
            # 🎯 DIÁLOGO DE SELEÇÃO DE PYTHON
            selected_python = self._show_python_version_dialog(python_versions, project_name)
            
            if selected_python is None:
                # Usuário cancelou, criar projeto básico
                self._create_basic_project_structure(project_full_path, project_name)
                return
    
            # ✅ CRIAR PROJETO COM PYTHON ESPECÍFICO
            python_path, python_version = selected_python
            self._create_project_with_python(project_full_path, project_name, python_path, python_version)
    
        except Exception as e:
            self.debug_log(f"❌ Erro ao criar projeto com seleção de Python: {e}", "ERROR")
            QMessageBox.warning(
                self, 
                "Erro", 
    ...
    def _detect_all_python_versions(self):
        """Detecta todas as versões do Python instaladas no sistema"""
        python_versions = []
        
        try:
            # Comandos para buscar Python no sistema
            commands_to_try = []
            
            if os.name == 'nt':  # Windows
                commands_to_try = ['python', 'py', 'python3']
                # Adicionar versões específicas
                for version in range(13, 5, -1):  # Python 3.13 até 3.6
                    commands_to_try.extend([
                        f'python3.{version}',
                        f'py -3.{version}',
                        f'python{version}',
                        f'C:\\Python3{version}\\python.exe',
                        f'C:\\Program Files\\Python3{version}\\python.exe'
                    ])
            else:  # Linux/Mac
                ...
                for version in range(13, 5, -1):
                    commands_to_try.extend([
                        f'python3.{version}',
                        f'/usr/bin/python3.{version}',
                        f'/usr/local/bin/python3.{version}'
                    ])
    
            # Testar cada comando
            for cmd in commands_to_try:
                try:
                    python_path = self._get_python_path_from_command(cmd)
                    if python_path and python_path not in [p[0] for p in python_versions]:
                        version_info = self._get_python_version_info(python_path)
                        if version_info:
                            python_versions.append((python_path, version_info))
                            self.debug_log(f"✅ Python detectado: {version_info}", "SUCCESS")
                except:
                    continue
    
            # Ordenar por versão (mais recente primeiro)
            python_versions.sort(key=lambda x: self._extract_version_number_from_info(x[1]), reverse=True)
            
            self.debug_log(f"🎯 Total de versões Python detectadas: {len(python_versions)}", "SUCCESS")
            return python_versions
            
        except Exception as e:
            self.debug_log(f"❌ Erro ao detectar versões Python: {e}", "ERROR")
            return []
    
    def _get_python_path_from_command(self, cmd):
        """Obtém o caminho real do Python a partir de um comando"""
        try:
            # Tentar obter o caminho executável
    ...
    def _get_python_version_info(self, python_path):
        """Obtém informações detalhadas da versão do Python"""
        try:
            result = subprocess.run(
                [python_path, '-c', 
                 'import sys; print(f"Python {sys.version_info.major}.{sys.version_info.minor}.{sys.version_info.micro} - {sys.executable}")'],
                capture_output=True, text=True, timeout=5
            )
            ...
            if result.returncode == 0:
                return f"{result.stdout.strip()} - {python_path}"
    ...
    def _extract_version_number_from_info(self, version_info):
        """Extrai número de versão para ordenação"""
        try:
            # Exemplo: "Python 3.9.5 - C:\Python39\python.exe"
            match = re.search(r'Python (\d+)\.(\d+)\.(\d+)', version_info)
            if match:
    ...
    def _show_python_version_dialog(self, python_versions, project_name):
        """Mostra diálogo para selecionar versão do Python"""
        try:
            dialog = QDialog(self)
            dialog.setWindowTitle(f"🐍 Selecionar Python para '{project_name}'")
            ...
            # Título
            title_label = QLabel(f"Selecione a versão do Python para o projeto '{project_name}':")
            title_label.setStyleSheet("font-weight: bold; font-size: 14px; margin: 10px; color: #569cd6;")
            layout.addWidget(title_label)
            
            # Informação
            info_label = QLabel("A versão selecionada será usada para:\n• Execução do projeto\n• Criação de virtualenv\n• Instalação de pacotes")
            info_label.setStyleSheet("font-size: 12px; margin: 10px; color: #858585; background-color: #2d2d30; padding: 8px; border-radius: 4px;")
            layout.addWidget(info_label)
            
            # Lista de versões Python
            ...
            # Adicionar versões à lista
            for python_path, version_info in python_versions:
                item = QListWidgetItem(f"🐍 {version_info}")
                item.setData(Qt.UserRole, (python_path, version_info))
                version_list.addItem(item)
            
            layout.addWidget(version_list)
            
            # Versão atual do sistema
            current_python = QLabel(f"Python atual do sistema: {sys.version}")
            current_python.setStyleSheet("font-size: 11px; margin: 5px; color: #569cd6;")
            layout.addWidget(current_python)
            ...
            btn_use_system = QPushButton("Usar Python do Sistema")
            btn_use_system.setStyleSheet("""
                QPushButton {
                    background-color: #387a44;
                    color: white;
                    border: none;
                    padding: 8px 16px;
                    border-radius: 4px;
            ...
            btn_use_selected = QPushButton("Usar Python Selecionado")
            btn_use_selected.setDefault(True)
            btn_use_selected.setStyleSheet("""
                QPushButton {
                    background-color: #007acc;
                    color: white;
                    border: none;
            ...
            button_layout.addWidget(btn_use_system)
            button_layout.addWidget(btn_use_selected)
            ...
            def on_use_system():
                nonlocal result
                result = (sys.executable, f"Python do Sistema - {sys.version}")
                dialog.accept()
            ...
            def on_use_selected():
                ...
                if current_item:
                    ...
                    QMessageBox.warning(dialog, "Seleção", "Selecione uma versão do Python.")
            ...
            btn_use_system.clicked.connect(on_use_system)
            btn_use_selected.clicked.connect(on_use_selected)
            
            # Selecionar primeiro item por padrão
            if version_list.count() > 0:
                version_list.setCurrentItem(version_list.item(0))
            ...
            self.debug_log(f"❌ Erro no diálogo de seleção: {e}", "ERROR")
            return None
    
    def _create_basic_project_structure(self, project_path, project_name):
    ...
    ''')
            ...
            raise Exception(f"Erro ao criar estrutura do projeto: {str(e)}")
    
    def _create_project_with_python(self, project_path, project_name, python_path, python_version):
        """Cria projeto com Python específico"""
        try:
            # Criar estrutura básica
            self._create_basic_project_structure(project_path, project_name)
            
            # Criar arquivo de configuração do Python
            config_file = os.path.join(project_path, ".python-version")
            with open(config_file, 'w', encoding='utf-8') as f:
                f.write(f"# Python configuration for {project_name}\n")
                f.write(f"# Selected: {python_version}\n")
                f.write(f"PYTHON_PATH={python_path}\n")
            
            # Perguntar sobre virtualenv
            reply = QMessageBox.question(
                self,
                "Virtualenv",
                f"Deseja criar um virtualenv para o projeto usando:\n\n{python_version}?",
                QMessageBox.Yes | QMessageBox.No,
                QMessageBox.Yes
            )
            
            if reply == QMessageBox.Yes:
                # Criar virtualenv com o Python selecionado
                self._create_venv_for_project(project_path, python_path, python_version)
            ...
            QMessageBox.information(
                ...
                f"🔧 Configuração salva em: .python-version"
            )
            
        except Exception as e:
            raise Exception(f"Erro ao criar projeto com Python: {str(e)}")
    
    def _create_venv_for_project(self, project_path, python_path, python_version):
        """Cria virtualenv para o projeto com Python específico"""
        try:
            ...
            if result.returncode == 0:
                if self.is_valid_venv(venv_path):
                    ...
                    self.debug_log(f"✅ Virtualenv criado: {venv_path}", "SUCCESS")
                else:
                    self.debug_log(f"⚠️ Virtualenv criado mas inválido: {venv_path}", "WARNING")
            else:
                error_msg = result.stderr if result.stderr else "Erro desconhecido"
                self.debug_log(f"❌ Erro ao criar virtualenv: {error_msg}", "ERROR")
                
        except Exception as e:
            self.debug_log(f"❌ Erro ao criar virtualenv para projeto: {e}", "ERROR")
    ...
    def _offer_python_selection_for_project(self, project_name):
        """Oferece seleção de Python durante a criação do projeto"""
        try:
            # 🎯 OBTER TODOS OS PYTHONS INSTALADOS
            available_pythons = self._get_installed_pythons()
            
            if not available_pythons:
                QMessageBox.information(
                    self,
                    "Python não encontrado",
                    "Nenhum Python encontrado. O virtualenv será criado com o Python padrão do sistema."
                )
                return None
            
            # ✅ VERIFICAR SE HÁ PYTHONS VÁLIDOS
            valid_pythons = [p for p in available_pythons if self._check_venv_module(p[0])]
            
            if not valid_pythons:
                QMessageBox.information(
                    self,
                    "Módulo venv não disponível",
                    "Nenhum Python com módulo 'venv' encontrado. "
                    "O virtualenv será criado com o método automático."
                )
                return None
            
            # 🎯 DIALOGO DE SELEÇÃO SIMPLIFICADO
            reply = QMessageBox.question(
                self,
                "Selecionar Python",
                f"Deseja selecionar uma versão específica do Python "
                f"para o projeto '{project_name}'?\n\n"
                f"✅ Pythons disponíveis: {len(valid_pythons)}\n"
                f"🎯 Recomendado para controle de versão",
                QMessageBox.Yes | QMessageBox.No,
                QMessageBox.Yes
            )
            
            if reply == QMessageBox.Yes:
                return self._show_python_selector_dialog(available_pythons)
            else:
                return None
                
        except Exception as e:
            self.debug_log(f"❌ Erro na oferta de seleção de Python: {e}", "ERROR")
            return None
    def _create_venv_for_new_project(self, project_path, python_exec, python_info):
        """Cria virtualenv para um novo projeto com Python específico"""
        try:
            venv_path = os.path.join(project_path, "venv")
            
            # Mostrar progresso
            progress = QProgressDialog(
                f"Criando virtualenv para novo projeto...\n\n"
                f"🐍 {python_info}\n"
                f"📁 {venv_path}",
                "Cancelar", 0, 0, self
            )
            progress.setWindowTitle("Configurando Projeto")
            progress.setModal(True)
            progress.show()
            QApplication.processEvents()
                ...
                if self.is_valid_venv(venv_path):
                    ...
                    if success:
                        QMessageBox.information(
                            self,
                            "Virtualenv Criado",
                            f"✅ Virtualenv configurado com sucesso!\n\n"
                            f"🐍 {python_info}\n"
                            f"📁 {venv_path}\n"
                            f"⚡ Ativado automaticamente"
                        )
                    else:
                        QMessageBox.warning(
                            self,
                            "Virtualenv Criado",
                            f"✅ Virtualenv criado mas não ativado:\n{venv_path}\n\n"
                            f"Ative manualmente quando necessário."
                        )
                else:
                    QMessageBox.warning(
                        self,
                        "Problema no Virtualenv",
                        f"⚠️ Virtualenv criado mas com problemas:\n{venv_path}\n\n"
                        f"Verifique a configuração manualmente."
                    )
            else:
                error_msg = result.stderr if result.stderr else "Erro desconhecido"
                QMessageBox.warning(
                    self,
                    "Erro no Virtualenv",
                    f"❌ Não foi possível criar o virtualenv:\n{error_msg}\n\n"
                    f"Você pode criá-lo manualmente depois."
                )
    ...
    def activate_venv_in_project(self, venv_path):
        """Ativa o venv no projeto E no terminal REAL"""
        try:
    ...
    def _detect_best_python_for_venv(self):
        """Detecta o melhor Python disponível para criar virtualenv"""
        try:
            python_candidates = []
    ...
    def _generate_python_commands(self):
        """Gera lista de comandos Python para testar"""
        python_commands = []
    ...
    def _has_valid_pythons(self, available_pythons):
        """Verifica se há Pythons com módulo venv disponível"""
        valid_count = 0
        for python_path, python_info in available_pythons:
            if self._check_venv_module(python_path):
                valid_count += 1
        return valid_count > 0
    
    def create_venv_with_python_selector(self):
        """Cria virtualenv permitindo escolher entre versões de Python instaladas"""
        try:
            if not self.project_path:
                QMessageBox.information(self, "Virtualenv", "Abra um projeto primeiro!")
                return False
            ...
            if not available_pythons:
                QMessageBox.critical(
                    self,
                    "Python não encontrado",
                    "❌ Nenhum Python encontrado no sistema.\n\n"
                    "Instale Python 3.6+ ou adicione ao PATH."
                )
                return False
            
            # ✅ VERIFICAR SE HÁ PYTHONS VÁLIDOS
            if not self._has_valid_pythons(available_pythons):
                QMessageBox.critical(
                    self,
                    "Módulo venv não encontrado",
                    "❌ Nenhum Python com módulo 'venv' disponível.\n\n"
                    "Instale o módulo venv ou selecione outro Python.\n\n"
                    "Soluções:\n"
                    "• Ubuntu/Debian: sudo apt-get install python3-venv\n"
                    "• Windows: Reinstale o Python marcando 'Install launcher'\n"
                    "• Mac: brew install python3"
                )
                return False
    
            # 🎯 DIALOGO PARA SELECIONAR PYTHON
            python_choice = self._show_python_selector_dialog(available_pythons)
            ...
            return self._create_venv_direct(python_exec, venv_path, python_info)
            
        except Exception as e:
            self.debug_log(f"❌ Erro no seletor de Python: {e}", "ERROR")
            QMessageBox.critical(self, "Erro", f"Erro ao criar virtualenv:\n{str(e)}")
            return False
            
    def _get_installed_pythons(self):
        """Obtém todos os Pythons instalados na máquina de forma robusta"""
        python_installations = []
        
        try:
            # 🎯 MÉTODO 1: Buscar em locais comuns de instalação
            common_paths = self._get_common_python_paths()
            
            for path in common_paths:
                if os.path.exists(path):
                    version_info = self._get_python_version_info(path)
                    if version_info:
                        python_installations.append((path, version_info))
                        self.debug_log(f"✅ Python encontrado: {path} -> {version_info}", "SUCCESS")
            
            # 🎯 MÉTODO 2: Usar comando where/which para encontrar no PATH
            path_pythons = self._find_pythons_in_path()
            for python_path, version_info in path_pythons:
                if python_path not in [p[0] for p in python_installations]:
                    python_installations.append((python_path, version_info))
                    self.debug_log(f"✅ Python no PATH: {python_path} -> {version_info}", "SUCCESS")
            
            # 🎯 MÉTODO 3: Buscar em registros do Windows (apenas Windows)
            if os.name == 'nt':
                windows_pythons = self._find_pythons_in_windows_registry()
                for python_path, version_info in windows_pythons:
                    if python_path not in [p[0] for p in python_installations] and os.path.exists(python_path):
                        python_installations.append((python_path, version_info))
                        self.debug_log(f"✅ Python no Registry: {python_path} -> {version_info}", "SUCCESS")
            
            # Ordenar por versão (mais recente primeiro)
            python_installations.sort(key=lambda x: self._extract_version_from_info(x[1]), reverse=True)
            
            self.debug_log(f"🎯 Total de Pythons encontrados: {len(python_installations)}", "SUCCESS")
            return python_installations
            
        except Exception as e:
            self.debug_log(f"❌ Erro ao buscar Pythons instalados: {e}", "ERROR")
            return []
    
    def _get_common_python_paths(self):
        """Retorna caminhos comuns onde o Python é instalado"""
        common_paths = []
        
        # Python do sistema atual
        if sys.executable:
            common_paths.append(sys.executable)
        
        if os.name == 'nt':  # Windows
            # Caminhos padrão do Windows
            base_paths = [
                r"C:\Python{version}\python.exe",
                r"C:\Program Files\Python{version}\python.exe",
                r"C:\Program Files (x86)\Python{version}\python.exe",
                os.path.expanduser(r"~\AppData\Local\Programs\Python\Python{version}\python.exe")
            ]
            
            # Versões do Python 3.6 até 3.12
            for version in range(36, 13, -1):  # 3.12 até 3.6
                version_str = f"3{version}" if version < 10 else f"3.{version}"
                for base_path in base_paths:
                    path = base_path.format(version=version_str)
                    common_paths.append(path)
                    
                    # Também adicionar versão sem ponto (Python312)
                    path_no_dot = base_path.format(version=f"3{version}")
                    common_paths.append(path_no_dot)
        
        else:  # Linux/Mac
            # Caminhos padrão do Linux/Mac
            base_paths = [
                "/usr/bin/python{version}",
                "/usr/local/bin/python{version}",
                "/opt/homebrew/bin/python{version}",
                "/usr/bin/python{version}m",  # Python com pymalloc
                "/usr/local/bin/python{version}m"
            ]
            
            # Versões do Python
            for version in range(6, 13):  # 3.6 até 3.12
    ...
    def _find_pythons_in_path(self):
        """Encontra Pythons disponíveis no PATH do sistema"""
        pythons_found = []
        
        try:
            if os.name == 'nt':  # Windows
                # Usar 'where' para encontrar python.exe
                result = subprocess.run(
                    ['where', 'python.exe'], 
                    capture_output=True, 
                    text=True,
                ...
                if result.returncode == 0:
                    ...
                    for path in paths:
                        ...
                        if path and os.path.exists(path):
                            version_info = self._get_python_version_info(path)
                            if version_info:
                                pythons_found.append((path, version_info))
            else:  # Linux/Mac
                # Usar 'which' e 'command -v' para encontrar python
                commands = ['which python3', 'command -v python3', 'which python', 'command -v python']
                for cmd in commands:
                    try:
                        result = subprocess.run(
                            cmd.split(), 
            ...
            self.debug_log(f"⚠️ Erro ao buscar Pythons no PATH: {e}", "WARNING")
        
        return pythons_found
    
    def _find_pythons_in_windows_registry(self):
        """Busca Pythons no registro do Windows (apenas Windows)"""
        if os.name != 'nt':
            return []
        ...
        try:
            ...
            # Chaves do registro onde o Python é normalmente registrado
            registry_keys = [
            ...
            for hkey, base_key in registry_keys:
                try:
                    with winreg.OpenKey(hkey, base_key) as key:
                        i = 0
                        while True:
                            try:
                                # Listar versões instaladas
                                version_key_name = winreg.EnumKey(key, i)
                                version_key_path = f"{base_key}\\{version_key_name}\\InstallPath"
                                
                                try:
                                    with winreg.OpenKey(hkey, version_key_path) as install_key:
                                        try:
                                            install_path, _ = winreg.QueryValueEx(install_key, "")
                                            python_exe = os.path.join(install_path, "python.exe")
                                            
                                            if os.path.exists(python_exe):
                                                version_info = self._get_python_version_info(python_exe)
                                                if version_info:
                                                    pythons_found.append((python_exe, version_info))
                                        except:
                                            pass
                                except:
                                    pass
            ...
            self.debug_log("⚠️ Módulo winreg não disponível", "WARNING")
        except Exception as e:
            self.debug_log(f"⚠️ Erro ao buscar no registro: {e}", "WARNING")
        
        return pythons_found
    
    def _get_python_version_info(self, python_path):
        """Obtém informações da versão do Python de forma segura"""
        try:
            result = subprocess.run(
                [python_path, "-c", 
                 "import sys; print(f'Python {sys.version_info.major}.{sys.version_info.minor}.{sys.version_info.micro} - {sys.executable}')"],
            ...
            if result.returncode == 0:
                ...
                # Fallback: tentar apenas --version
                result = subprocess.run(
                    [python_path, "--version"],
                    capture_output=True,
                    text=True,
            ...
            self.debug_log(f"⚠️ Erro ao obter versão de {python_path}: {e}", "WARNING")
        
        return None
    
    def _extract_version_from_info(self, version_info):
        """Extrai número de versão para ordenação"""
        try:
            # Exemplo: "Python 3.9.5 - C:\Python39\python.exe"
            match = re.search(r'Python (\d+)\.(\d+)\.(\d+)', version_info)
    ...
    def _show_python_selector_dialog(self, available_pythons):
        """Mostra diálogo para selecionar Python - VERSÃO MELHORADA"""
        try:
            # Criar diálogo personalizado
            dialog = QDialog(self)
            dialog.setWindowTitle("🎯 Selecionar Python para Virtualenv")
            ...
            # Título
            title_label = QLabel("Selecione a versão do Python para criar o virtualenv:")
            title_label.setStyleSheet("font-weight: bold; font-size: 14px; margin: 10px; color: #569cd6;")
            layout.addWidget(title_label)
            
            # Informações
            info_label = QLabel("✅ Python com módulo 'venv' disponível\n❌ Python sem módulo 'venv'")
            info_label.setStyleSheet("font-size: 12px; margin: 5px; color: #858585;")
            layout.addWidget(info_label)
            
            # Lista de Pythons
            python_list = QListWidget()
            python_list.setAlternatingRowColors(True)
            python_list.setStyleSheet("""
                QListWidget {
                    background-color: #1e1e1e;
                    color: #d4d4d4;
                    border: 1px solid #3e3e42;
            ...
            # Adicionar Pythons à lista
            enabled_count = 0
            for python_path, python_info in available_pythons:
                item = QListWidgetItem()
                
                has_venv = self._check_venv_module(python_path)
                
                if has_venv:
                    item.setText(f"✅ {python_info}")
                    item.setForeground(QColor("#90ee90"))  # Verde claro
                    item.setToolTip(f"✅ Módulo 'venv' disponível\n📁 {python_path}")
                    enabled_count += 1
                else:
                    item.setText(f"❌ {python_info}")
                    item.setForeground(QColor("#ff6b6b"))  # Vermelho claro
                    item.setToolTip(f"❌ Módulo 'venv' não disponível\n📁 {python_path}")
                    item.setFlags(item.flags() & ~Qt.ItemIsEnabled)  # Desabilitar item
                
                item.setData(Qt.UserRole, (python_path, python_info, has_venv))
                python_list.addItem(item)
            
            layout.addWidget(python_list)
            
            # Contador
            count_label = QLabel(f"Pythons disponíveis: {enabled_count}/{len(available_pythons)}")
            count_label.setStyleSheet("font-size: 12px; margin: 5px; color: #858585;")
            ...
            btn_ok = QPushButton("Usar Python Selecionado")
            ...
            def update_ok_button():
                """Atualiza estado do botão OK baseado na seleção"""
                current_item = python_list.currentItem()
            ...
            def on_accept():
                nonlocal selected_python
                current_item = python_list.currentItem()
                if current_item and (current_item.flags() & Qt.ItemIsEnabled):
                    selected_python = current_item.data(Qt.UserRole)
                    dialog.accept()
                else:
                    QMessageBox.warning(
                        dialog, 
                        "Seleção Inválida", 
                        "Selecione um Python com módulo 'venv' disponível."
                    )
            ...
            python_list.itemDoubleClicked.connect(on_double_click)
            python_list.currentItemChanged.connect(on_selection_changed)
            
            # Estado inicial
            update_ok_button()
            
            # Selecionar primeiro item habilitado, se existir
            for i in range(python_list.count()):
                item = python_list.item(i)
                if item.flags() & Qt.ItemIsEnabled:
                    python_list.setCurrentItem(item)
                    break
            
            # Executar diálogo
            if dialog.exec() == QDialog.Accepted and selected_python:
                python_path, python_info, has_venv = selected_python
                return (python_path, python_info)
    ...
    def _create_venv_direct(self, python_exec, venv_path, python_info):
        """Cria virtualenv usando caminho direto do Python"""
        try:
            self.debug_log(f"🎯 Criando venv com: {python_info}", "INFO")
            
            # ✅ VERIFICAÇÕES DE SEGURANÇA
            if not os.path.exists(python_exec):
                QMessageBox.critical(self, "Erro", f"Python não encontrado:\n{python_exec}")
                return False
            ...
            if not self._check_venv_module(python_exec):
                QMessageBox.critical(
                    self,
                    "Módulo venv não disponível",
                    f"❌ O Python selecionado não tem módulo 'venv':\n{python_info}\n\n"
                    f"Selecione outro Python ou instale o módulo venv."
                )
                return False
            ...
            if os.path.exists(venv_path):
                ...
                if reply == QMessageBox.Yes:
                    try:
                        ...
                        QMessageBox.critical(self, "Erro", f"Erro ao remover venv:\n{str(e)}")
                        return False
                else:
                    if self.is_valid_venv(venv_path):
                        return self.activate_venv_in_project(venv_path)
            ...
            # ✅ CRIAR VIRTUALENV DIRETAMENTE
            try:
                self.debug_log(f"🔨 Executando: {python_exec} -m venv {venv_path}", "INFO")
                
                # Usar o caminho absoluto direto
                result = subprocess.run(
                    [python_exec, "-m", "venv", venv_path],
                    capture_output=True,
                    text=True,
                ...
                if result.returncode == 0:
                    ...
                    import time
                    time.sleep(2)  # Dar tempo para o sistema
                    
                    if self.is_valid_venv(venv_path):
                        self.venv_path = venv_path
                        self.debug_log(f"✅ Virtualenv criado com sucesso: {venv_path}", "SUCCESS")
                    ...
                    QMessageBox.critical(
                        ...
                        f"Comando: {python_exec} -m venv {venv_path}"
                    )
                    return False
            ...
            self.debug_log(f"❌ Erro inesperado: {e}", "ERROR")
            QMessageBox.critical(
                self, 
                "Erro inesperado", 
                f"❌ Erro inesperado:\n{str(e)}"
            ...
            return False
# Adicione este método na classe IDE, na seção de métodos de projeto e virtualenv

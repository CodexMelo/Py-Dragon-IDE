from PySide6.QtWidgets import (QPlainTextEdit, QWidget, QVBoxLayout, QMenu, 
                              QMessageBox, QFileDialog, QListWidget, QListWidgetItem, QTextEdit, QHBoxLayout)
...
from PySide6.QtGui import (QFont, QTextCursor, QSyntaxHighlighter, QTextCharFormat, 
                          QColor, QPainter, QPalette, QTextFormat, QPen)
...
class UnifiedCodeEditor(QPlainTextEdit):
    ...
    def setup_visual_indicators(self):
        ...
        self.setup_vertical_guideline()  # NOVA LINHA ADICIONADA
    ...
    def get_enhanced_suggestions(self):
        ...
        try:
            suggestions.update(self.get_language_keywords())
            suggestions.update(self.extract_local_definitions())
    ...
    def setup_vertical_guideline(self):
        ...
        self.guideline_position = 80  # Posição padrão em 80 caracteres
        self.guideline_color = QColor(60, 60, 80, 120)  # Cor suave
        
        # Timer para atualizar a linha quando necessário
        ...
        # Conecta sinais para atualizar a linha
    ...
    def update_guideline(self):
        """Atualiza a linha vertical de orientação - VERSÃO SIMPLIFICADA"""
        if not self.guideline_enabled:
            return
            
        try:
            # Força o redesenho da viewport
    ...
    def paintEvent(self, event):
        ...
        # Primeiro chama o paintEvent original
        super().paintEvent(event)
        
        # Depois desenha a linha vertical se estiver habilitada
        if self.guideline_enabled:
            self.draw_vertical_guideline(event)

    def draw_vertical_guideline(self, event):
        """Desenha a linha vertical diretamente no viewport - VERSÃO CORRIGIDA"""
        try:
            painter = QPainter(self.viewport())
            painter.setPen(QPen(self.guideline_color, 1))
            
            # Calcula a posição baseada na largura do caractere
            char_width = self.fontMetrics().horizontalAdvance(' ')
            guideline_x = self.guideline_position * char_width
            
            # Verifica se a linha está dentro da área visível
            viewport_width = self.viewport().width()
            if guideline_x > viewport_width:
                return
                
            # Desenha a linha vertical em toda a altura
    ...
    def get_last_visible_block(self):
        """Método auxiliar para obter o último bloco visível - CORREÇÃO DO ERRO"""
        try:
            # Obtém a geometria do viewport
            viewport_rect = self.viewport().rect()
            bottom = viewport_rect.bottom()
            
            # Encontra o bloco na posição inferior
    ...
    def fix_indentation(self):
        """Corrige indentação do código Python de forma inteligente - APENAS TEXTO COLADO"""
        try:
            cursor = self.textCursor()
            if not cursor.hasSelection():
                return
                
            # Obtém apenas o texto selecionado (colado)
            ...
            # Processa apenas as linhas selecionadas
            start_pos = cursor.selectionStart()
            end_pos = cursor.selectionEnd()
            
            # Encontra o bloco inicial e final da seleção
            ...
            # Substitui apenas as linhas selecionadas
    ...
    def _fix_line_indentation(self, line_text, line_number):
        ...
        # Lógica básica de indentação para Python
        if line_number > 0:
            prev_block = self.document().findBlockByNumber(line_number - 1)
            if prev_block.isValid():
                prev_line = prev_block.text().rstrip()
                
                # Aumenta indentação se linha anterior termina com dois pontos
                if prev_line.endswith(':'):
                    return '    ' + stripped
                
                # Mantém indentação similar se linha anterior tem código
    ...
    # 1. SELEÇÃO DE OCORRÊNCIAS SEMELHANTES - CORRIGIDO
    def select_all_occurrences(self):
        """Seleciona todas as ocorrências da palavra sob o cursor - VERSÃO CORRIGIDA"""
        cursor = self.textCursor()
        
        # Se não há seleção, seleciona a palavra atual
        if not cursor.hasSelection():
    ...
    # 2. TAB CORRIGIDO - NÃO DUPLICA TEXTO
    def keyPressEvent(self, event):
        """Manipulação de teclas - VERSÃO CORRIGIDA"""
        try:
            key = event.key()
            modifiers = event.modifiers()
            
            # Ctrl+D - Seleciona ocorrências semelhantes
            if modifiers == Qt.ControlModifier and key == Qt.Key_D:
                self.select_all_occurrences()
                event.accept()
                return
                
            # Escape - Limpa seleções
            elif key == Qt.Key_Escape:
                self.clear_selections()
                if hasattr(self, 'autocomplete_widget') and self.autocomplete_widget.isVisible():
                    self.autocomplete_widget.hide()
                event.accept()
                return
                
            # Tab/Shift+Tab - Indentação inteligente
            elif key == Qt.Key_Tab or key == Qt.Key_Backtab:
                if self._handle_indentation_key(event):
                    return
                    
            # Ctrl+V - Cola com formatação apenas do texto colado
    ...
    def _indent_selection(self):
        """Adiciona indentação à seleção - VERSÃO CORRIGIDA"""
        cursor = self.textCursor()
        if not cursor.hasSelection():
            return
    ...
    def _unindent_selection(self):
        """Remove indentação da seleção - VERSÃO CORRIGIDA"""
        cursor = self.textCursor()
        if not cursor.hasSelection():
            return
    ...
    # 3. COLA COM FORMATAÇÃO APENAS DO TEXTO COLADO
    def _paste_with_formatting(self):
        """Cola e formata apenas o texto colado"""
        # Cola o conteúdo normalmente
        self.paste()
    ...
    def _format_pasted_text(self):
        ...
        try:
            ...
            # Aplica formatação apenas na seleção (texto colado)
            self.fix_indentation()
            
        except Exception as e:
            print(f"❌ Erro ao formatar texto colado: {e}")

    # 4. GUIAS DE INDENTAÇÃO - CORRIGIDO
    ...
    def update_indentation_guides(self):
        """Atualiza as linhas de orientação para indentação - VERSÃO CORRIGIDA"""
        ...
        try:
            ...
            # Configurações das guias
            tab_width = 4
            max_guides = 8
            
            # Obtém blocos visíveis CORRETAMENTE
            first_visible_block = self.firstVisibleBlock()
            if not first_visible_block.isValid():
                return
                
            # Usa o novo método para obter o último bloco visível
            last_visible_block = self.get_last_visible_block()
            
            # Itera pelas linhas visíveis
            ...
            while block.isValid() and block.blockNumber() <= last_visible_block.blockNumber():
                ...
                # Adiciona guias para múltiplos níveis de indentação
                for level in range(1, max_guides + 1):
                    guide_pos = level * tab_width
                    
                    # Só mostra guia se estiver além da indentação atual e dentro do texto
                    if guide_pos > indent_level and guide_pos < len(line_text):
                        # Cria seleção para a guia
                        selection = QTextEdit.ExtraSelection()
            ...
            # Aplica as guias (mantém outras seleções)
    ...
    def show_context_menu(self, position):
        ...
        # NOVA AÇÃO: Alternar linha vertical
